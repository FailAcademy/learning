...
It's a good idea to store values that are used throughout our game in the `settings.js` file. Let's add a constant for the SVG namespace:
...
Comment-out the SVG mark-up in `index.html` (but keep it for reference for now!), and create the `<svg>` element that will wrap the game components in the `Game` class `render` method:
...
How do we import the `SVG_NS` into `Game.js`?
